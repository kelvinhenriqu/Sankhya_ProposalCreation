...
exibe dias corridos entre `Cabecalho.DataProposta` e a data de entrega retornada pela
API, como `60 dias após colocação do pedido`; `false` (padrao) exibe a data final, como `20/10/2026`.
Entregas anteriores a data da proposta resultam em dias negativos. Valores vazios ou que nao sejam
datas sao preservados. No modo de dias, uma data de proposta ausente ou invalida
impede o calculo e retorna erro. Reinicie a aplicacao ao alterar o `.env`.
